...
final public class FetchedResultsControllerPublisher<FetchType> where FetchType : NSFetchRequestResult {
    ...
    public convenience init(request: NSFetchRequest<FetchType>, context: NSManagedObjectContext) {
        let controller = NSFetchedResultsController(fetchRequest: request, managedObjectContext: context, sectionNameKeyPath: nil, cacheName: nil)
        self.init(fetchedResultsController: controller, performFetchNotRequired: false)
    }
